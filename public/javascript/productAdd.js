...
window.addEventListener('load' , () => {
    let formProductCreate = qs('form-productCreate'),
        ...
        price = qs('#price'),
        autorId = qs('#autor'),
        publisherId = qs('#publisher'),
        genreId = qs('#genre'),
        languageId = qs('#language'),
        categoryId = qs('#category'),
        description = qs('#description'),
        errorName = qs('#errorName'),
        errorPrice = qs('#errorPrice'),
        errorAutorId = qs('#errorAutor'),
        errorPublisherId = qs('#errorPublisher'),
        errorGenreId = qs('#errorGenre'),
        errorLanguageId = qs('#errorLanguage'),
        errorCategoryId = qs('#errorCategory'),
    ...
    autorId.addEventListener('blur' , () => {
        switch (true) {
            case autorId.value.length == 0:
                autorId.classList.add('is-invalid');
                errorAutorId.innerHTML = 'Debes indicar el nombre del Autor';
                errores = true
                break;
            case autor.value.length <=5:
                autorId.classList.add('is-invalid');
                errorAutorId.innerHTML = 'Deberá tener al menos 5 caracteres';
                errores = true
            default:
    ...
    publisherId.addEventListener('blur' , () => {
        switch (true) {
            case publisherId.value.length == 0:
    ...
    languageId.addEventListener('blur' , () => {
        switch (true) {
            case languageId.value.length == 0:
    ...
    categoryId.addEventListener('blur' , () => {
        switch (true) {
            case categoryId.value.length == 0:
